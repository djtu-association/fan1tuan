...
$(document).ready(function(){
	...
	this.shop_AjaxChange = function(pNumber){
		...
		var data = {"pSize":pSize,"pNumber":pNumber,"open":open,"accord":accord,"order":order,"shopType":shopType,"shopAffairTagIds":shopAffairTagIds,"shopTasteTagIds":shopTasteTagIds};
	...
	var shop_createPanel = function(json){
		...
		for(var i=0,size=json.shopGeo.length; i<size; i++){
			...
			"<p class='text text-success'><small>营业</small>：<label class='label label-info'>"+data[i].content.openTime+" - "+data[i].content.closeTime+"</label></p>"+//营业时间data[i].content.openTime  data[i].content.closeTime
